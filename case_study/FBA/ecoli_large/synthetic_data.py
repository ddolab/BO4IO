...
def FBA(RSet, MetSet, UB_rand, LB_rand, S_dict, redox_rxn_tuple, c_ref, n_rxn = 3, scen = 1, ind = 1):
    ...
    obj_rxns_tup = [[(-1,'BIOMASS_Ec_iML1515_core_75p37M')], [(-1,'ATPM')], [(-1,'EX_glc__D_e')], [(-1,'EX_etoh_e')], redox_rxn_tuple]
    obj_rxns_lst = ['abs_norm','BIOMASS_Ec_iML1515_core_75p37M','ATPM','EX_glc__D_e','EX_etoh_e','REDOX POTENTIAL']
    model = ConcreteModel()
...
def generate_random_bounds(RSet, UB_dict_WT, LB_dict_WT, numSam = 10):
    # LB_dict_WT['BIOMASS_Ec_iML1515_core_75p37M'] = 0.005
    ...
    for i in range(len(lhdnormc_UB[0, :])):
        ...
        # elif RSet[i] == 'BIOMASS_Ec_iML1515_core_75p37M':
