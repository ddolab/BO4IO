...
# indice length, for example: for all i in 1,...,I
...
# model.Tt = pyo.Set(within=model.l*model.l, initialize = [(i, j) for i in model.l for j in model.l if i != j]) #GPool
model.Tt = pyo.Set(dimen = 2)
...
model.Pl = pyo.Param(model.j, model.k,mutable=True)
model.Pu = pyo.Param(model.j, model.k,mutable=True)

# indexed parameters for Gen Pool
model.crinit = pyo.Param(model.i)
model.crpool = pyo.Param(model.l)
model.crx = pyo.Param(model.Tx)
...
# Gen Pool Variables
model.rinit = pyo.Var(model.i, domain=pyo.Binary, initialize = 1)
model.rpool = pyo.Var(model.l, domain=pyo.Binary, initialize = 1)
...
def obj_expression(m):
    return sum((m.c[i]+ m.crx[i,l]) * m.x[i,l]  for (i,l) in m.Tx) - sum(m.dy[l,j] * m.y[l,j] for j in m.j for l in m.l) - (sum((m.dz[i,j] - m.c[i])*m.z[i,j] for (i,j) in m.Tz)) + sum(m.crinit[i]*m.rinit[i] for i in m.i) + sum(m.crpool[l]*m.rpool[l] for l in m.l)


# assign the objective function expression as the model objective function
model.OBJ = pyo.Objective(rule=obj_expression, sense=pyo.minimize)


# constraints
def feed_avail_lower(m,i):
    return m.Al[i] * m.rinit[i] <= sum(m.x[i,l] for l in m.l if (i,l) in m.Tx) + sum(m.z[i,j] for j in m.j if (i,j) in m.Tz)
model.feedavaillowerconstraint = pyo.Constraint(model.i, rule = feed_avail_lower)

def feed_avail_upper(m,i):
    return m.Au[i] * m.rinit[i] >= sum(m.x[i,l] for l in m.l if (i,l) in m.Tx) + sum(m.z[i,j] for j in m.j if (i,j) in m.Tz)
model.feedavailupperconstraint = pyo.Constraint(model.i, rule = feed_avail_upper)

def pool_capacity(m,l):
    return sum(m.x[i,l] for i in m.i if (i,l) in m.Tx) <= m.S[l] * m.rpool[l]
...
def hard_bound_y(m,l,j):
    return m.y[l,j] <= min(pyo.value(m.S[l]),pyo.value(m.Du[j]), sum(pyo.value(m.Au[i]) for (i,ll) in m.Tx))*pyo.value(m.rpool[l])
model.hardbound_y = pyo.Constraint(model.l, model.j, rule = hard_bound_y)

def hard_bound_x(m,i,l):
    return m.x[i,l] <= min(pyo.value(m.Au[i]), pyo.value(m.S[l]), sum(pyo.value(m.Du[j]) for j in m.j))*min(pyo.value(m.rinit[i]), pyo.value(m.rpool[l]))
...
def loss_rule(m):
    return sum((m.x[i,l]-m.xRef[i,l])**2 for (i,l) in m.Tx)
